...
class BaseSettings:
    ...
    def _setUp(
        ...
        mocker.patch("tv_runner.DOMAIN", "http://mediaviewer:8000")
        mocker.patch("utils.DOMAIN", "http://mediaviewer:8000")
        mocker.patch("tv_runner.MINIMUM_FILE_SIZE", 100)
        self.local_paths = self._local_tv_paths()
        mocker.patch("tv_runner.LOCAL_TV_SHOWS_PATHS", self.local_paths)
...
class TestRun(BaseSettings):
    ...
    def setUp(self, mocker):
        ...
        mocker.patch("tv_runner.UNSORTED_PATHS", [str(self.unsorted_dir)])
    ...
    def test_unsorted(self, create_video_file):
        ...
        unsorted_video_file = create_video_file(self.unsorted_dir, filename)
...
class TestPostMediaFile(BaseSettings):
    def _local_tv_paths(self):
        return [str(self._temp_directory / f"{x}") for x in range(3)]

    def test_post_media_file(self):
        post_resp = Tv.post_media_path(self.tv_dir)
        media_path_id = post_resp["pk"]
        mf_resp = MediaFile.post_media_file(
            self.video_file.name, media_path_id, self.media_file_size
        )
    ...
    def test_multiple_media_paths(self, create_tv_directory):
        ...
        for idx, dir_str in enumerate(self.local_paths):
            ...
            if tv_name is None:
                tv_dir, video_file = create_tv_directory(directory=new_dir)
                tv_name = tv_dir.name
            else:
            ...
            MediaFile.post_media_file(video_file.name, resp["pk"], self.media_file_size)
            if tv is None:
                tv = resp["tv"]

        resp = Tv.get_tv(tv)
        media_paths = {x["path"] for x in resp["media_paths"]}
