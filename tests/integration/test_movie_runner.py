...
class BaseSettings:
    ...
    def setUp(
        ...
        mocker.patch("movie_runner.DOMAIN", "http://mediaviewer:8000")
        self.local_paths = self._local_movie_paths()
        mocker.patch("movie_runner.LOCAL_MOVIE_PATHS", self.local_paths)
...
class TestPostMediaFile(BaseSettings):
    ...
    def test_multiple_media_paths(self, create_movie_directory):
        ...
        for idx, dir_str in enumerate(self.local_paths):
            ...
            movie_dir, video_file = create_movie_directory(directory=new_dir)
